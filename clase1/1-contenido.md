...
- La clase `Short` define que todos los carros tendrán un color, una marca, una velocidad, etc., y podrán arrancar, acelerar, etc.
...
- **Relación Clase-Objeto**: La clase es la definición abstracta; el objeto es la materialización concreta de esa definición. Puedes crear muchos objetos a partir de una sola clase, y cada objeto tendrá su propio estado independiente.
  ...
  ```mermaid
  graph TD
      A[Clase Short] --> B(Objeto miShort);
      A --> C(Objeto otroShort);
      B -- "titulo: vacaciones\nautor: creador" --> D(Estado);
      B -- "reproducir()\npausar()" --> E(Comportamiento);
      C -- "titulo: programar\nautor: DevSenior" --> F(Estado);
      C -- "reproducir()\npausar()" --> G(Comportamiento);
  ```
